...
def registrar(motor, modelo, entrada, salida, latencia_ms):
    ...
    with conexion() as con, con.cursor() as cur:
        cur.execute(
            "INSERT INTO inferencias (motor, modelo, entrada, tipo, latencia_ms) "
            "VALUES (%s, %s, %s, %s, %s)",
...
def clasificar_eco(texto: str) -> str:
    """Motor por reglas: línea base sin modelo."""
...
def clasificar_ollama(texto: str) -> str:
    ...
    prompt = (
        ...
        + ", ".join(TIPOS)
        + ". Responde únicamente con la palabra de la categoría, sin explicaciones.\n"
        + f"Mensaje: {texto}\nCategoría:"
    )
